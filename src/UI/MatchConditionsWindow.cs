...
public class MatchConditionsWindow : Sts2ModalPanel
{
    ...
    public static MatchConditionsWindow ShowFor(StartRunLobby lobby)
    {
        ...
        // button template (BuildSettingsActionButton) and exact 320x64 size as MatchmakingWindow's own "대기" row
        // (its "이어하기" one) rather than BuildTextActionButton's flat-color pill, per request - reads as one more
        // "Send Feedback"-style settings-screen action instead of a kick/ban-style moderation button.
        Button copyLinkButton = BuildSettingsActionButton(Loc.Get("복사"), new Vector2(320f, 64f), out Label copyLinkLabel);
        WireHoverBrightness(copyLinkButton);
        copyLinkButton.Pressed += () => OnCopyInviteLinkPressed(lobby, copyLinkLabel);
        pages[0].AddChild(BuildSettingsRow(Loc.Get("링크 복사"), copyLinkButton,
            tooltip: Loc.Get("이 방에 바로 참가할 수 있는 링크를 클립보드에 복사합니다."), overlayParent: panel));
