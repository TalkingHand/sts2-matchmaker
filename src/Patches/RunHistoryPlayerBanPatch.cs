...
public static class RunHistoryPlayerBanPatch
{
    ...
    public static void ReadyPostfix(NRunHistory __instance)
    {
        try
        {
            ...
            banButton.Pressed += () =>
            {
                ...
                if (IsLocalPlayer(__instance, targetId))
                {
    ...
    public static void SelectPlayerPostfix(NRunHistory __instance, NRunHistoryPlayerIcon playerIcon)
    {
        try
        {
            ...
            banButton.Visible = !IsLocalPlayer(__instance, targetId);
    ...
    private static void PositionBanButton(NRunHistory instance, Button banButton)
    {
        try
        {
            ...
            banButton.Visible = !(selectedId.HasValue && IsLocalPlayer(instance, selectedId.Value));
    ...
    /// <summary>RunHistoryPlayer.Id is NOT always the real SteamID64 - confirmed by decompiling sts2.dll's
    /// NetSingleplayerGameService: its NetId is a hardcoded 1uL regardless of platform (its Platform property still
    /// reports PlatformType.Steam whenever Steam is initialized, which is the normal case), so a solo run's lone
    /// player entry is saved with Id 1, never the player's actual SteamID. Only genuine coop, connected through the
    /// real SteamHost/SteamClient transport, sets NetId to SteamUser.GetSteamID().m_SteamID. Since a solo run's
    /// history always has exactly one player entry (see NRunHistory.DisplayRun's own player-count check), and that
    /// entry can only ever be the local player, Players.Count == 1 is what actually identifies "this is me" for
    /// solo saves - falls back to the SteamID64 comparison (the original, still-correct check) for everything else.</summary>
    private static bool IsLocalPlayer(NRunHistory instance, ulong targetId)
    {
        RunHistory? history = Traverse.Create(instance).Field("_history").GetValue<RunHistory>();
        if (history != null && history.Players.Count == 1)
        {
            return true;
        }
        return targetId == SteamUser.GetSteamID().m_SteamID;
    }
}
